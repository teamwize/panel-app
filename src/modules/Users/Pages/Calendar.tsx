...
import { DayOffLeaveTypeJson, DayOffStatusJson, DayOffLeaveTypeColor, DayOffStatusColor } from '../../../constants';
        ...
        <main className='px-4'>
          ...
          <div>
            ...
            {isWorkingDay ? selectedDateRequests
              ...
              .map((request) => <RequesItem calculateDistance={calculateDistance} request={request} key={request.id} />) : ''}
          </div>
...
type RequesItemProps = {
  request: DayOffResponse;
  calculateDistance: (startAt: string, endAt: string) => number;
};

function RequesItem({ request, calculateDistance }: RequesItemProps) {
