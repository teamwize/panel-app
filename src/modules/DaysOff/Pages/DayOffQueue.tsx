        ...
        <div className='md:w-4/5 w-full overflow-y-auto mb-2 fixed top-16 md:top-0 bottom-0 right-0 h-screen'>
            <div className='pt-5 md:mx-auto md:w-full md:max-w-[70%]'>
                <div className="flex items-center border-b border-gray-200 dark:border-gray-800 pb-4 mb-2">
                    ...
                    <h1 className="text-lg md:text-xl font-semibold text-indigo-900 dark:text-indigo-200">Day Off
                        Queue</h1>
                </div>
                ...
                {requestsList
                    ...
                    .map((request) => (
                        <Request key={request.id} request={request} onClick={handleRowClick}
                                 calculateDistance={calculateDistance}/>
                    ))}

                {requestsList.length > recordsPerPage && (
                    <Pagination pageSize={recordsPerPage} pageNumber={currentPage} setPageNumber={setCurrentPage}
...
function Request({request, onClick, calculateDistance}: RequestType) {
    ...
    return (
        <div>
            <section onClick={() => onClick(request)}
                     className='flex items-center text-indigo-900 dark:text-indigo-200 mb-2 pb-2 border-b border-gray-200 dark:border-gray-800 cursor-pointer'>
                <img className="h-10 w-10 rounded-full mr-2"
                     src="https://upload.wikimedia.org/wikipedia/commons/0/09/Man_Silhouette.png" alt="User avatar"/>

                <div className='flex flex-col md:flex-row w-full md:justify-between'>
                    <p className="fullname text-sm font-semibold mb-1 md:mb-0 md:w-1/3">{request.user.firstName} {request.user.lastName}</p>
                    ...
                    <div className='flex items-center justify-between md:w-2/3 w-full'>
                        <div className='flex'>
                            ...
                            <p className='distance md:text-sm text-xs text-indigo-800 dark:text-indigo-300'>
                                ({dayjs(request.endAt).diff(request.startAt, 'day') + 1} {distance === 1 ? "Day" : "Days"})
                            </p>
                        </div>
